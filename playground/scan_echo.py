...
class ScanProcessor(Node):
    ...
    def scan_callback(self, scanMsg):
        distance_threshold = 0.5  # Limit the distance to 0.7m
        angle_detection = 65  # Limit the angle to 65°

        # Angles in rads of -45° et 45°
        angle_min = -angle_detection * (math.pi / 180)  # -π/4
        angle_max = angle_detection * (math.pi / 180)  # π/4

        # Indexes of the angles
        ...
        for i in range(index_min, index_max + 1):
            if 0.05 < scanMsg.ranges[i] < distance_threshold:
                ...
                aDistance = float(scanMsg.ranges[i])  # Explicit conversion in float 
                aPoint = Point32()
                aPoint.x = float(math.cos(angle) * aDistance)  # Explicit conversion in float
                aPoint.y = float(math.sin(angle) * aDistance)  # Explicit conversion in float
                aPoint.z = 0.0  # Make sure it is a 2D point
